...
// /** ✅ include only if has actual value (per your rule) */
...
//   // strings/radio/select/date/email/phone/textarea...
...
//       if (response?.status) {
//         // ✅ Toast success (custom + centered + 5s + close)
//         showSubmissionToast({
...
//   sortActiveSections,
//   buildDefaultValues,
//   scrollToFirstError, // ✅ now exported
//   OTHER_VALUE,
//   otherFieldName,
//   isOtherTextareaKey,
// } from "../utils/form.builder";
...
//         // ignore __other textarea keys (never send them directly)
//         // (they are not in schema fields, but safe anyway)
//         if (isOtherTextareaKey(f.name)) continue;

//         // select: if Other => replace with textarea text
//         if (f.type === "select" && v === OTHER_VALUE) {
//           const otherKey = otherFieldName(f.name);
//           const otherText = formValues[otherKey];

//           if (typeof otherText === "string" && otherText.trim()) {
//             valuesPayload[f.name] = otherText.trim();
//           }
//           continue;
//         }
