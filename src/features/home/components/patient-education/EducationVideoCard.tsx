...
const EducationVideoCard: React.FC<Props> = ({ video, index }) => {
  ...
  const initial: TargetAndTransition | undefined = shouldReduceMotion
    ? undefined
    : { opacity: 0, y: 16 };

  const transition: Transition | undefined = shouldReduceMotion
    ? undefined
    : { duration: 0.35, ease: "easeOut", delay: 0.04 * index };

  const animate: TargetAndTransition | undefined = shouldReduceMotion
    ? undefined
        ...
        {open && parsed.embed ? (
          ...
          >
            <motion.div
              ...
              animate={
                shouldReduceMotion ? undefined : { y: 0, scale: 1, opacity: 1 }
              }
              exit={
                shouldReduceMotion
                  ? undefined
