...
import { SimpleArticle } from "@/features/blogs/types/blog.types";
interface CategoriesSidebarProps {
  isOpen: boolean;
  onClose: () => void;
}

const EducationSidebar: React.FC<CategoriesSidebarProps> = ({
  isOpen,
  onClose,
}) => {
  const { i18n, t } = useTranslation();
  const dir = i18n.dir() as "ltr" | "rtl";
  const { isLoading, data } = useGetSimpleBlogs();

  const titleId = "education-system-sidebar-title";
  ...
  return (
    <>
      <Backdrop
        isOpen={isOpen}
        onClick={onClose}
        aria={t("A11y.closeEducationsNav", "Close Education navigation")}
      />
      ...
      >
        ...
        >
          <ul
            ...
            className="flex flex-col divide-y divide-[color:var(--border-subtle)] p-3"
          >
            {isLoading && (
            ...
            {!isLoading && data?.length
              ? data.map((cat: SimpleArticle, index) => (
                  <li className="text-sm py-4" key={index} role="none">
                    <Link
                      onClick={onClose}
                      to={`/patient-education/${cat?.slug}`}
                    >
                      {cat?.title}
                    </Link>
                  </li>
...
export default memo(EducationSidebar);
